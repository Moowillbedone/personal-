...
def main() -> int:
    ...
    # Daily retention: bound every unbounded-growth table so the free-tier
    # 500MB DB cap is never hit (2026-07 sustainability audit — see db.py
    # for the per-table growth math). Each prune is fail-soft.
    for label, fn in (
        (f"price_snapshots >{db.PRICE_SNAPSHOT_KEEP_DAYS}d", db.prune_price_snapshots),
        (f"signals >{db.SIGNALS_KEEP_DAYS}d", db.prune_signals),
        (f"ai_analysis.context >{db.AI_CONTEXT_KEEP_FULL_DAYS}d", db.slim_ai_analysis_context),
        (f"ai_analysis >{db.AI_ANALYSIS_KEEP_DAYS}d", db.prune_ai_analysis),
    ):
        try:
            n = fn(sb)
            print(f"retention [{label}]: {n} rows affected")
        except Exception as e:
            print(f"WARN: retention [{label}] failed (non-fatal): {e}", file=sys.stderr)
    return 0
